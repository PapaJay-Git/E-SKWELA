        ...
        if (in_array($fileActualExt, $allowed)) {
          if ($fileError === 0) {
            if ($fileSize < 30000000) {
                $fileNameNew = "parent_profile_".$hashed."$hashedDate".".".$fileActualExt;
                ...
                if(file_exists($fileDestination)){
                  ...
                  if(move_uploaded_file($fileTmpName, $fileDestination)){
                    $add_file = "UPDATE parents SET profile = ? WHERE parent_id =?;";
                    ...
                    if(!mysqli_stmt_prepare($stmt, $add_file)) {
                      ...
                      //run sql
                      mysqli_stmt_bind_param($stmt, "si",  $fileDestination, $row['parent_id']);
