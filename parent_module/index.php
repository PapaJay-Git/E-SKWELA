...
<?php
  ...
  $sql = "SELECT * FROM parent_student WHERE parent_id = $parent_id;";
  ...
  ?>
    ...
    <h2 class="head" style="margin-top:40px; margin-bottom:20px"></h2>
         ...
         <h1 class="card-title">Student Grades</h1>
             <hr size="4" width="100%" color="grey">
<?php
while ($parent_row = mysqli_fetch_assoc($parent_result)) {
  $student_id = $parent_row['student_id'];
  $sql2 = "SELECT * FROM student WHERE student_id = $student_id;";
  $parent_result2 = $conn->query($sql2);
  $row = mysqli_fetch_assoc($parent_result2);
 ?>
      <div class="card">
       <div class="card-header">
         <?php echo 'Name - '.$row['l_name'].", ".$row['f_name'].'<br>LRN - '.$row['school_id'];  ?>
       </div>
       <div class="card-body"><!-- BODY-->
          <div class="row">
            <div class="table-responsive">
             <table id="classdata<?php echo $student_id;  ?>" class="table table-hover table-xxlg table-bordered">
          ...
          ?>
                ...
                <!-- for tables design, functions and printing output -->
                 ...
                 $(document).ready(function() {
                     $('#classdata<?php echo $student_id;  ?>').DataTable( {
                       dom: 'Blfrtip',
