...
if (isset($_POST['read']) && isset($_POST['notif_id']) ) {
  ...
  {
  $notif_id = $fn[$e];
  $sql_grade = "SELECT * FROM student_notification WHERE id = ? AND student_id = ?;";
  $stmt = mysqli_stmt_init($conn);
   ...
   if ($result->num_rows == 0) {
     ...
     header("location: notification.php?success=id_not_exist");
     exit();
   }
  }
  for($i=0; $i < $N2; $i++)
  ...
  $read = "read";
  $notif_id = $fn[$i];
  $sql_grade = "UPDATE student_notification SET status = ? WHERE id =? AND student_id =?;";
  $stmt = mysqli_stmt_init($conn);
    mysqli_stmt_prepare($stmt, $sql_grade);
    mysqli_stmt_bind_param($stmt, "sii", $read, $notif_id, $student_id);
    mysqli_stmt_execute($stmt);
  }

 $_SESSION['success']= "$N notification successfully marked as read!";
 header("location: notification.php?notset");
 exit();
  ...
  $read = "read";
  $notif_id3 = $fn[$i];
  $sql_grade = "DELETE FROM student_notification WHERE id =? AND student_id =?;";
  $stmt = mysqli_stmt_init($conn);
    mysqli_stmt_prepare($stmt, $sql_grade);
    mysqli_stmt_bind_param($stmt, "ii", $notif_id3, $student_id);
    mysqli_stmt_execute($stmt);
  }
