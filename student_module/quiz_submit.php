...
$sql333 = "SELECT used_attempt, deadline_date, max_attempt FROM student_quiz where quiz_id = $quiz_id AND student_id = $student_id;";
...
//Updating the questions
  if(isset($_POST['TF_id'])){
    ...
    $tf_id = $tf[$i];
    $sql = "SELECT * FROM quiz_question WHERE quiz_id=? AND quiz_question_id =?";
    $stmt = mysqli_stmt_init($conn);
    $id = $_GET['tc_id'];
      if(!mysqli_stmt_prepare($stmt, $sql) || !mysqli_stmt_bind_param($stmt, "ii", $quiz_id, $tf_id) || !mysqli_stmt_execute($stmt)) {
            $_SESSION['error'] = "SQL error";
            header("location: quiz.php?sql=error");
            exit();
      }
      $result = mysqli_stmt_get_result($stmt);
      $row = mysqli_fetch_assoc($result);
      $check1 = "SELECT quiz_id, COUNT(*) AS count FROM student_quiz_answer WHERE quiz_question_id =? AND student_id =? GROUP BY answer_id";
      $stmt = mysqli_stmt_init($conn);
      $id = $_GET['tc_id'];
        ...
        if($row3['count'] == 0){
          $check2 = "INSERT INTO student_quiz_answer (quiz_question_id, student_id, quiz_id, quiz_type_id, answer_1, points_1, score) values (?, ?, ?, ?, ?, ?, ?);";
          $stmt = mysqli_stmt_init($conn);
            ...
            mysqli_stmt_bind_param($stmt, "iiiisii", $tf_id, $_SESSION['student_session_id'], $quiz_id, $type, $tf_answer, $points_tf, $score);
            mysqli_stmt_execute($stmt);
        }else{
          $check2 = "UPDATE student_quiz_answer SET answer_1= ?,points_1= ?, score = ? WHERE quiz_question_id = ? AND student_id= ?";
          $stmt = mysqli_stmt_init($conn);
          $id = $_GET['tc_id'];
            if(!mysqli_stmt_prepare($stmt, $check2)){
                  $_SESSION['error'] = "SQL error";
  ...
  if(isset($_POST['multi_id'])){
    ...
    $stmt = mysqli_stmt_init($conn);
    $id = $_GET['tc_id'];
      if(!mysqli_stmt_prepare($stmt, $sql) || !mysqli_stmt_bind_param($stmt, "ii", $quiz_id, $multi_id) || !mysqli_stmt_execute($stmt)) {
            $_SESSION['error'] = "SQL error";
            header("location: quiz.php?sql=error");
            exit();
      }
      ...
      $stmt = mysqli_stmt_init($conn);
        ...
        if($row3['count'] == 0){
          ...
          $id = $_GET['tc_id'];
            ...
            mysqli_stmt_bind_param($stmt, "iiiisii", $multi_id, $_SESSION['student_session_id'], $quiz_id, $type, $multi_answer, $points_multi, $score);
            mysqli_stmt_execute($stmt);
        }else{
          $check2 = "UPDATE student_quiz_answer SET answer_1= ?,points_1= ?, score = ? WHERE quiz_question_id = ? AND student_id= ? AND quiz_id= ? ";
          $stmt = mysqli_stmt_init($conn);
            if(!mysqli_stmt_prepare($stmt, $check2)){
                  $_SESSION['error'] = "SQL error";
                  header("location: quiz.php?sql=error");
            ...
            mysqli_stmt_bind_param($stmt, "siiiii", $multi_answer, $points_multi, $score, $multi_id, $_SESSION['student_session_id'], $quiz_id);
            mysqli_stmt_execute($stmt);
        }
    }
  }
  if(isset($_POST['enum_id'])){
    ...
    $stmt = mysqli_stmt_init($conn);
    $id = $_GET['tc_id'];
      if(!mysqli_stmt_prepare($stmt, $sql) || !mysqli_stmt_bind_param($stmt, "ii", $quiz_id, $enum_id) || !mysqli_stmt_execute($stmt)) {
            $_SESSION['error'] = "SQL error";
            header("location: quiz.php?sql=error");
            exit();
      }
      ...
      $stmt = mysqli_stmt_init($conn);
        ...
        $row3 = mysqli_fetch_assoc($result2);
        if($row3['count'] == 0){
          $check2 = "INSERT INTO student_quiz_answer (quiz_question_id, student_id, quiz_id, quiz_type_id, answer_1, answer_a_2,
          ...
          $id = $_GET['tc_id'];
            ...
            mysqli_stmt_bind_param($stmt, "siiisssssiiiiii", $enum_id, $_SESSION['student_session_id'], $quiz_id, $type, $answer_1, $answer_2, $answer_3,
            $answer_4, $answer_5, $points_1, $points_2, $points_3, $points_4, $points_5, $score);
            mysqli_stmt_execute($stmt);
        }else{
          $check2 = "UPDATE student_quiz_answer SET answer_1=?, answer_a_2=?, answer_b_3=?, answer_c_4=?, answer_d_5=?, points_1=?,
          points_2=?, points_3=?, points_4=?, points_5=?, score=? WHERE quiz_question_id = ? AND student_id= ? AND quiz_id= ? ";
          $stmt = mysqli_stmt_init($conn);
            if(!mysqli_stmt_prepare($stmt, $check2)){
                  $_SESSION['error'] = "SQL error";
                  header("location: quiz.php?sql=error");
            ...
            $points_4, $points_5, $score, $enum_id, $_SESSION['student_session_id'], $quiz_id);
            mysqli_stmt_execute($stmt);
        }
      }
    }
  if(isset($_POST['iden_id'])){
      ...
      $stmt = mysqli_stmt_init($conn);
      $id = $_GET['tc_id'];
        if(!mysqli_stmt_prepare($stmt, $sql) || !mysqli_stmt_bind_param($stmt, "ii", $quiz_id, $iden_id) || !mysqli_stmt_execute($stmt)) {
              $_SESSION['error'] = "SQL error";
              header("location: quiz.php?sql=error");
              exit();
        }
        ...
        $stmt = mysqli_stmt_init($conn);
          ...
          if($row3['count'] == 0){
            ...
            $id = $_GET['tc_id'];
              ...
              mysqli_stmt_bind_param($stmt, "iiiisii", $iden_id, $_SESSION['student_session_id'], $quiz_id, $type, $iden_answer, $points_iden, $score);
              mysqli_stmt_execute($stmt);
          }else{
            $check2 = "UPDATE student_quiz_answer SET answer_1= ?,points_1= ?, score = ? WHERE quiz_question_id = ? AND student_id= ? AND quiz_id= ? ";
            $stmt = mysqli_stmt_init($conn);
              ...
              mysqli_stmt_bind_param($stmt, "siiiii", $iden_answer, $points_iden, $score, $iden_id, $_SESSION['student_session_id'], $quiz_id);
              mysqli_stmt_execute($stmt);
          }
        }
      }
  //Updating the quiz for students
  $sql = "SELECT * FROM student_quiz_answer WHERE student_id=? AND quiz_id =? AND quiz_type_id != ?";
  $stmt = mysqli_stmt_init($conn);
  $essayt = 5;
    if(!mysqli_stmt_prepare($stmt, $sql) || !mysqli_stmt_bind_param($stmt, "iii", $_SESSION['student_session_id'], $quiz_id, $essayt) || !mysqli_stmt_execute($stmt)) {
          $_SESSION['error'] = "SQL error";
          exit();
    }
    ...
    $querrye = "SELECT * FROM student_quiz WHERE student_id=? AND quiz_id =?";
    $stmt = mysqli_stmt_init($conn);
      if(!mysqli_stmt_prepare($stmt, $querrye) || !mysqli_stmt_bind_param($stmt, "ii", $_SESSION['student_session_id'], $quiz_id) || !mysqli_stmt_execute($stmt)) {
            $_SESSION['error'] = "SQL error";
            exit();
      }
      ...
      $sql = "UPDATE student_quiz SET used_attempt=?, total_score=?, submit_date=?, start_time=? WHERE quiz_id=? AND student_id =?";
      $stmt = mysqli_stmt_init($conn);
      if(!mysqli_stmt_prepare($stmt, $sql) || !mysqli_stmt_bind_param($stmt, "iissii", $attemptss, $question_score, $date,  $start_time, $quiz_id, $_SESSION['student_session_id']) || !mysqli_stmt_execute($stmt)) {
            $_SESSION['error'] = "SQL error";
            exit();
      }
      $notification = "SELECT * FROM quiz where quiz_id =?; ";
      $stmt = mysqli_stmt_init($conn);
      //Preparing the prepared statement
        ...
        $notification = "SELECT * FROM teacher_notification where quiz_id =? AND teacher_id =?; ";
        $stmt = mysqli_stmt_init($conn);
        //Preparing the prepared statement
          mysqli_stmt_prepare($stmt, $notification);
          ...
          if ($notif->num_rows > 0) {
            ...
            $notification = "INSERT INTO teacher_notification (teacher_id, teacher_class_id, date_given, type, quiz_id) VALUES (?, ?, ?, ?, ?)";
            $stmt = mysqli_stmt_init($conn);
            //Preparing the prepared statement
            $type_notif = "quiz";
              mysqli_stmt_prepare($stmt, $notification);
              mysqli_stmt_bind_param($stmt, "iissi",  $teacher_id, $tc_id, $date, $type_notif, $quiz_id);
              mysqli_stmt_execute($stmt);
          }
      $_SESSION['success'] = "Submit Success! All questions are now been graded.";
      header("location: quiz_open.php?tc_id=$tc_id&quiz_id=$quiz_id&success");
      exit();
}else {
