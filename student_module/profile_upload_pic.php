...
if(isset($_POST['change_profile'])) {
      ...
      if (!exif_imagetype($fileTmpName)) {
        $_SESSION['error'] = "Only jpg, jpeg, png and gifs are allowed.";
        ...
        if (in_array($fileActualExt, $allowed)) {
          if ($fileError === 0) {
            if ($fileSize < 30000000) {
                $fileNameNew = "student_profile_".$hashed."$hashedDate".".".$fileActualExt;
                $fileDestination =  '../Admin_module/profile_pics/'.$fileNameNew;
                if(file_exists($fileDestination)){
                  $_SESSION['error'] = "This file name already exist";
                  header("Location: profile_pic_change.php?file_id=&already_exist=file");
                  exit();
                }else{
                  if(move_uploaded_file($fileTmpName, $fileDestination)){
                    $add_file = "UPDATE student SET profile = ? WHERE student_id =?;";
                    ...
                    if(!mysqli_stmt_prepare($stmt, $add_file)) {
                      ...
                      //run sql
                      mysqli_stmt_bind_param($stmt, "si",  $fileDestination, $row['student_id']);
