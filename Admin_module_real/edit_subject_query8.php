...
if (isset($_POST['subject_id_8']) && isset($_POST['name_8']) && isset($_POST['update_8'])) {
  $name = strtoupper(ltrim(htmlspecialchars($_POST['name_8'])));
  $id = $_POST['subject_id_8'];
  if (empty($name)) {
    $_SESSION['error']= "Error, name cannot be empty";
    header("location: edit_grade8.php?subject_id=$id");
    exit();
  }
    $seven = 8;
    $title = $name." ".$seven;
    ...
    if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
      ...
      header("location: edit_grade8.php?subject_id=$id&sqlerror");
      exit();
    }
      //run sql
 ...
 if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
   ...
   header("location: edit_grade8.php?subject_id=$id&success=sqlerror");
   exit();
 }
   //run sql
   mysqli_stmt_bind_param($stmt, "ssii", $name, $title, $id, $seven);
      if(!mysqli_stmt_execute($stmt)){
        ...
        echo $conn->error;
        header("location: edit_grade8.php?subject_id=$id&success=error");
        exit();
      }
      $_SESSION['success'] = "Subject Name is now Updated!";
      header("Location:  edit_grade8.php?subject_id=$id&update=success");
      exit();
}
else{
    $_SESSION['error'] = "Submission is not settled properly.";
    header("Location edit_grade8.php?subject_id=$id&button");
    exit();
}
