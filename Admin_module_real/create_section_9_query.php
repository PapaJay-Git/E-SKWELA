...
if (!isset($_POST['add_section_9']) || !isset($_POST['section_name_9'])) {

  $_SESSION['error']= "You cannot add an empty section name!";
   header("location: create_section_9.php?notset");
   exit();
 }else{
   $s = $_POST['section_name_9'];
   $N = count($s);
   $N2 = count($s);
   ...
   if (empty($section)) {
     ...
     header("location: create_section_9.php?error=number");
     exit();
   }
     $seven = 9;
     ...
     if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
       ...
       header("location: create_section_9.php?success=sqlerror");
       exit();
     }
       //run sql
   ...
   for($i=0; $i < $N; $i++)
   {
         ...
         if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
           ...
           mysqli_stmt_bind_param($stmt, "ssii", $secs, $title, $seven, $ste);
              if(!mysqli_stmt_execute($stmt)){
                ...
                echo $conn->error;
                header("location: create_section_9.php?&success=error");
                exit();
              }
       }else{
     ...
     $_SESSION['success']= "$N Grade 9 section added successfully!";
     header("location: create_section_9.php?success=true");
     exit();
 }
