      ...
      if ($new_password == $confirm_password) {
        if ($length < 21 && $length > 5) {
            if (preg_match('/^\w+$/', $new_password)) {
                  $ee = $_SESSION['admin_session_id'];
                  $queryT = "SELECT password FROM admin where admin_id = $ee;";
                  $adminCheck = $conn->query($queryT);
                  ...
                  if(password_verify($old_password, $password)) {
                    ...
                    if(!mysqli_stmt_prepare($stmt, $sql)) {
                      ...
                      exit();
                        ...
                        mysqli_stmt_bind_param($stmt, "si", $new_hashed, $_SESSION['admin_session_id']);
