...
if (isset($_POST['add_grade9']) && isset($_POST['subject_name']) && !isset($_POST['STE'])) {
   ...
   if (empty($subject)) {
     ...
     header("location: create_grade9.php?error=number");
     exit();
   }
     $seven = 9;
     ...
     if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
       ...
       header("location: create_grade9.php?success=sqlerror");
       exit();
     }
       //run sql
    ...
    if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
      ...
      mysqli_stmt_bind_param($stmt, "ssi", $subject2, $title, $seven);
         if(!mysqli_stmt_execute($stmt)){
           ...
           echo $conn->error;
           header("location: create_grade9.php?&success=error");
           exit();
         }

   }
     $_SESSION['success']= "$N Grade 9 subject added successfully!";
     header("location: create_grade9.php?success=true");
     exit();
 }elseif (isset($_POST['add_grade9']) && isset($_POST['subject_name']) && isset($_POST['STE'])) {
   $s = $_POST['subject_name'];
   $N = count($s);
   $N2 = count($s);
   ...
   if (empty($subject)) {
    ...
    $ste = 1;
     ...
     if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
       ...
       $result_check = mysqli_stmt_get_result($stmt);
       if ($result_check->num_rows > 0) {
           $_SESSION['error']= "It looks like there is already an STE additional subject for grade 9. Only 1 STE additional subject is valid. If you wish to change, just edit the current grade 9 STE additional subjects name!";
           header("location: create_grade9.php?success=sqlerror");
           exit();
       }
       $sql_grade = "SELECT * FROM subjects WHERE subject_code =? AND grade = ?;";
   ...
   for($i=0; $i < $N; $i++)
   {
    ...
    if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
     ...
     $_SESSION['success']= "Additional STE Grade 9 subject added successfully!";
     header("location: create_grade9.php?success=true");
     exit();
 }else{
     $_SESSION['error']=  "You cannot add an empty subject name!";
    header("location: create_grade9.php?notset");
    exit();
  }
