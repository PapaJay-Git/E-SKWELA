...
?>
          <div class="container-xxl">
            <h2 style="margin-top:40px; margin-bottom:20px" class="head" id="h2"><p id="pp" style="display:inline">STUDENTS FROM STE THAT IS NOW IN GRADE 10</h2>

            <hr size="4" width="100%" color="grey">
        <form class="transfer_form" id="transfer"action="previous_10_query.php" method="post">
                  ...
                  <a href="previous.php" class="btn btn-danger">BACK</a>
                  </div>
                    <select id="card2"class="form-control" name="section_id">
                      <option default selected value disabled>-- GRADE 10 SECTIONS YOU CAN TRANSFER TO--</option>
              ...
              <table id="myidata2" class="display responsive" style="width:100%">
                  ...
                  $sql = "SELECT * FROM student WHERE class_id = ? AND previous_ste != ?;";
                  $stmt = mysqli_stmt_init($conn);
                  mysqli_stmt_prepare($stmt, $sql);
                  mysqli_stmt_bind_param($stmt, "ii", $sections['class_id'], $zero);
                  mysqli_stmt_execute($stmt);
                  $result2 = mysqli_stmt_get_result($stmt);
                  while ($students = mysqli_fetch_assoc($result2)) {
                    $sql5 = "SELECT * FROM class where class_id =?";
                    $stmt = mysqli_stmt_init($conn);
                    mysqli_stmt_prepare($stmt, $sql5);
                    mysqli_stmt_bind_param($stmt, "i", $students['previous_ste']);
                    mysqli_stmt_execute($stmt);
                    $result99 = mysqli_stmt_get_result($stmt);
                    $old = mysqli_fetch_assoc($result99);
                  ...
                  $(document).ready(function() {
                      $('#myidata2').DataTable( {
                        ...
                        lengthMenu: [[100, -1], [100, "All"]],
                         ...
                         {
                           extend: 'print',
                           title: 'Previous STE 10 List id:<?php $rand = substr(uniqid('', true), -5); echo $rand;?>'
                         },
                         {
                           extend: 'excelHtml5',
                           title: 'Previous STE 10 List id:<?php $rand = substr(uniqid('', true), -5); echo $rand;?>'
