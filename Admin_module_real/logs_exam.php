<?php
  ...
  $exam = $conn->query($checkClasses11);

?>
<div class="container">
  <h2 class="head" id="text" style="margin-top:40px; margin-bottom:20px">Created Exams</h2>
                  <hr size="4" width="100%" color="grey">
                      <button type="button" onclick="goBack()" style="float:right; width: 100px;" class="btn btn-primary">BACK</button>
                      <div class="row" style="margin-bottom: 10px;">
                        <h1>&nbsp</h1>
                      </div>
                  <div class="table-responsive" style="display: block; margin-top: 10px;" id="exams">
                  <table id="myidata" style="width:100%">
                      ...
                      while ($exam_row = mysqli_fetch_assoc($exam)) {
                        ...
                        $checkClasses11 = "SELECT * FROM exam_question where exam_id = $exam_id;";
                        ...
                        if ($exam_row['published'] == 1) {
                          $published = "Yes";
                        }else {
                          $published = "No";
                        }
                        if (strtotime($exam_row['upload_date'])) {
                          $timestamp = strtotime($exam_row['upload_date']);
                          $upload = date("F j Y, g:i a", $timestamp);
                        }else {
                          $today = "Unreadable";
                        }
                        if (strtotime($exam_row['start_date'])) {
                          $timestamp = strtotime($exam_row['start_date']);
                          $start = date("F j, g:i a", $timestamp);
                        }else {
                          $start = "Unreadable";
                        }
                        if (strtotime($exam_row['deadline_date'])) {
                          $timestamp = strtotime($exam_row['deadline_date']);
                       ...
                       <tr>
                         ...
                         <td><?php echo $exam_row['max_attempt']; ?></td>
                         <td><?php echo $exam_row['timer']." mins"; ?></td>
