...
if (!isset($_POST['add_teacher']) || !isset($_POST['first_name']) || !isset($_POST['last_name']) ||!isset($_POST['password']) || !isset($_POST['school_id'])) {

    $_SESSION['error']= "You cannot add an empty name or password!";
   header("location: create_teachers.php?notset");
   exit();
 }else{
   $fn = $_POST['first_name'];
   $ln = $_POST['last_name'];
   ...
   if ($length < 21 && $length > 5) {
     ...
     header("location: create_teachers.php?error=number");
     exit();
   }
   ...
   $sql_grade = "SELECT * FROM teachers WHERE school_id=?;";
   ...
   if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
     ...
     if ($result_check->num_rows > 0) {
         $_SESSION['error']= "It looks like $school_id2 is already been added before. You cannot duplicate teacher numbers!";
         header("location: create_teachers.php?error=number");
         exit();
     }
   }
   for($i=0; $i < $N; $i++)
   {
     ...
     if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
       ...
       $result_check = mysqli_stmt_get_result($stmt);
       if ($result_check->num_rows > 0) {
           $_SESSION['error']= "It looks like $school_id2 is already been added before. You cannot duplicate school IDs!";
           header("location: create_teachers.php?error=number");
           exit();
       }
    $new_hashed = password_hash($password2, PASSWORD_DEFAULT);
    $sql_grade = "INSERT INTO teachers (school_id, f_name, l_name, password) VALUE (?, ?,?,?);";
    $stmt = mysqli_stmt_init($conn);
    //Preparing the prepared statement
    if(!mysqli_stmt_prepare($stmt, $sql_grade)) {
      $_SESSION['error']= "Error adding teacher";
      header("location: create_teachers.php?success=sqlerror");
      exit();
    }
      //run sql
      mysqli_stmt_bind_param($stmt, "isss", $school_id, $first_name, $last_name, $new_hashed);
         if(!mysqli_stmt_execute($stmt)){
           ...
           echo $conn->error;
           header("location: create_teachers.php?&success=error");
           exit();
         }

   }
     $_SESSION['success']= "$N Teacher account added successfully!";
     header("location: create_teachers.php?success=true");
     exit();
 }
