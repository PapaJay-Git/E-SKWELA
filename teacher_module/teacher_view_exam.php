...
<h2 class="head" style="margin-top:40px;">EXAMS</h2>
...
$sql = "SELECT * FROM exam where teacher_class_id =? AND teacher_id=?";
    ...
    $result = mysqli_stmt_get_result($stmt);
      ...
      <div class="row">
        ...
        <div class="col-md-4 btn-group" role="group">
          <a href='teacher_create_exam.php?tc_id=<?php echo $row['teacher_class_id']; ?>'style="width: 120px;" class="btn btn-primary" >CREATE EXAM</a>
          <a style="width: 30px" href="teacher_exam.php" class="btn btn-danger">BACK</a>
        </div>
      </div>
      ...
      while($row = mysqli_fetch_assoc($result)){
        $str = mb_strimwidth($row['exam_title'], 0, 28, "...");
      ?><tr>
         ...
         <td><?php if ($row['published'] == 1) {?>
                      <a href='exam_published.php?exam_id=<?php echo $row['exam_id'].'&tc_id='.$id; ?>'  class="btn btn-danger">OFF</a><?php
                    }else{?>
                       <a href='exam_published.php?exam_id=<?php echo $row['exam_id'].'&tc_id='.$id; ?>'  class="btn btn-primary">ON</a><?php
                      }?></td>
         <td><a href='exam_submissions.php?exam_id=<?php echo $row['exam_id'].'&tc_id='.$id; ?>'  class="btn btn-primary">VIEW</a></td>
         <td><a href='exam_add_question.php?exam_id=<?php echo $row['exam_id'].'&tc_id='.$id; ?>'  class="btn btn-primary">EDIT</a></td>
         <td>
            <button type="button" onclick="ConfirmDelete('exam_delete_query.php?exam_id=<?php echo $row['exam_id'].'&tc_id='.$id; ?>')" class="btn btn-danger">Delete</button>
         </td>
        </tr>
      <?php
        }
...
$(document).ready(function() {
    $('#aexample').DataTable( {
      ...
      lengthMenu: [[10, 25, 50, 100, -1], [10, 25, 50, 100, "All"]],
       ...
       {
         extend: 'print',
         title: 'Created Exam id:<?php $rand = substr(uniqid('', true), -5); echo $rand?>'
       },
       {
         extend: 'excelHtml5',
         title: 'Created Exams id:<?php $rand = substr(uniqid('', true), -5); echo $rand;?>'
       }]
  } );
} );
 </script>
