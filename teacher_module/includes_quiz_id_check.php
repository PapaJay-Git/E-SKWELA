...
exit();
	...
	if (!isset($_GET['tc_id']) || !is_numeric($_GET['tc_id'])){
		...
		header("location: teacher_quiz.php?tc_id=notset");
			exit();
    //echo "not set";
		}else{
      ...
      $sql = "SELECT * FROM teacher_class WHERE teacher_class_id=? AND teacher_id=?;";
		    ...
		    if(!mysqli_stmt_prepare($stmt, $sql)) {
				      ...
				      header("location: teacher_quiz.php?sql=error");
              exit();
			  } else {
            ...
            $result = mysqli_stmt_get_result($stmt);
              if ($result->num_rows > 0) {
                ...
                //error in url id is missing go back to homepage
								...
								$_SESSION['error'] = "Access on Quizzes, Denied!";
                header("location: teacher_quiz.php?id_url=no_match");
										exit();
                //echo "no equal data";
			        }
            }
