...
$exam = $_GET['exam_id'];
	if (!isset($_GET['student_id']) || !is_numeric($_GET['student_id'])){
		...
		header("location: exam_submissions.php?tc_id=$id&exam_id=$exam&notset");
		  exit();
    //echo "not set";
		}else{
			//checking if the parameters are valid
      $sqls = "SELECT * FROM student_exam WHERE exam_id=? AND teacher_id=? AND student_id =?;";
			$stmt = mysqli_stmt_init($conn);
      if(!mysqli_stmt_prepare($stmt, $sqls)) {
              $_SESSION['error'] = "SQL error, please contact tech support.";
				      header("location: exam_submissions.php?tc_id=$id&exam_id=$exam&sql=error");
              exit();
			  } else {
          $student_id = $_GET['student_id'];
    				mysqli_stmt_bind_param($stmt, "iii", $examid, $_SESSION['teacher_session_id'], $student_id);
            ...
            $result_stu = mysqli_stmt_get_result($stmt);
              if ($result_stu->num_rows > 0) {
               ...
               $student_sub = mysqli_fetch_assoc($result_stu);
                ...
                header("location: exam_submissions.php?tc_id=$id&exam_id=$exam&examid=no_match");
								exit();
                //echo "no equal data";
			        }
            }
