<div id="create_question" style="display: none; margin-top:10px;">
  <h5>Question Creation</h5>
    <label for="cars">Exam Types:</label>
    <select name="examChosenType" id="cars" onchange="leaveChange(this)">
  <option disabled selected value> -- options -- </option><?php
  // loop for dropdown option,
  while ($valueType = mysqli_fetch_assoc($examTypes)) {?>
  <option value="<?php echo $valueType['exam_type_id']; ?>"><?php echo $valueType['type_title'] ?></option><?php
  }?>
  </select>
  <!-- True or false form -->
  <?php $one = 1; ?>
  <form class="form-horizontal" action="exam_add_query.php?view_id=<?php echo $one ?>&exam_id=<?php echo $examiddd ?>&tc_id=<?php echo $iddd ?>" method="post" id="TF" style="display: none;">
  ...
  <form class="form-horizontal" action="exam_add_query.php?view_id=<?php echo $one ?>&exam_id=<?php echo $examiddd ?>&tc_id=<?php echo $iddd ?>" method="post" id="multi" style="display: none;">
...
<!-- Enumeration form -->
  <form class="form-horizontal" action="exam_add_query.php?view_id=<?php echo $one ?>&exam_id=<?php echo $examiddd ?>&tc_id=<?php echo $iddd ?>" method="post" id="enume" style="display: none;">
  ...
  <!-- Identification form -->
  <form class="form-horizontal" action="exam_add_query.php?view_id=<?php echo $one ?>&exam_id=<?php echo $examiddd ?>&tc_id=<?php echo $iddd ?>" method="post" id="iden" style="display: none;">
  ...
  <!-- essay form -->
  <form class="form-horizontal" action="exam_add_query.php?view_id=<?php echo $one ?>&exam_id=<?php echo $examiddd ?>&tc_id=<?php echo $iddd ?>" method="post" id="essay" style="display: none;">
    <label>Essay</label><br>
    <label>Question</label><br>
    <p><label style="font-size: 11px;">Limit 2500 characters</label></p>
    <textarea class="ckeditor" id="mytext" name="IDNQ2" style="width: 100%; height: 100px;" required></textarea><br>
    <label>Max score</label>
    <input type="number" name="IDENpoints" min="1" max="100" onpaste="return false;" onDrag="return false" onDrop="return false" autocomplete="off" placeholder="points" required></p>
    <input type="submit" class="btn-success" name="Essay_Submit" value="Save">
  </form>
</div>
