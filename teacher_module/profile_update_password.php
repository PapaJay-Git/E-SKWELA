      ...
      if ($new_password == $confirm_password) {
        if ($length < 21 && $length > 5) {
            if (preg_match('/^\w+$/', $new_password)) {
                  $ee = $_SESSION['teacher_session_id'];
                  $queryT = "SELECT password FROM teachers where teacher_id = $ee;";
                  $teacherCheck = $conn->query($queryT);
                  $teacher = mysqli_fetch_assoc($teacherCheck);
                  ...
                  if(password_verify($old_password, $password)) {
                    ...
                    if(!mysqli_stmt_prepare($stmt, $sql)) {
                      ...
                      exit();
                        ...
                        mysqli_stmt_bind_param($stmt, "si", $new_hashed, $_SESSION['teacher_session_id']);
